...
import { db } from "@/db";
import { promos, clients } from "@/db/schema";
      ...
      {allPromos.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground/60 mt-1">
              Create daily specials, happy hour promos, or advertisements.
            </p>
          </CardContent>
        </Card>
      ) : (
        <Card>
          <Table>
            ...
            <TableBody>
              {allPromos.map((promo) => {
                const activateAction = updatePromoStatus.bind(
                  null,
                  promo.id,
                  "active"
                );
                const cancelAction = updatePromoStatus.bind(
                  null,
                  promo.id,
                  "cancelled"
                );
                ...
                return (
                  ...
                  >
                    <TableCell>
                      <Badge
                        variant={statusColors[promo.status]}
                        className="text-[10px]"
                      >
                        {promo.status}
                      </Badge>
                    ...
                    <TableCell>
                      ...
                      >
                        {typeLabels[promo.promoType] || promo.promoType}
                      </Badge>
                    </TableCell>
                    <TableCell>
